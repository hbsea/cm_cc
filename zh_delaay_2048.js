...
$.ajax({
    ...
    async: true,
    success: function(data) {
        $('.rule-item-content').append((new Date().getMinutes()) + ":" + (new Date().getSeconds()) + ":  saveMenuClickstatus:" + data.status + " desc:" + data.desc + type + "<br>");
        var params = {
            'channelId': channelId,
            'userPhone': userPhone,
            'operationType': type
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                // WT.si_x = 20 点击办理
                Webtrends.multiTrack({
                    argsa: ['WT.si_n', si_n, 'WT.si_s', si_s, 'WT.si_x', '20']
                });
                layer.closeAll();
                if (data.status == 200) {
                    //秒杀成功弹框
                    $('.ts-suc-span1').text('手速超人！');
                    $('.ts-suc-span2').text('OK____OK:    ' + data.desc);
                    $('.public - box ').show();
                    $('.suc_ts_div ').show();
                    $('#p1_seckill ').hide();
                    $('#p2_seckill ').hide();
                    ...
                    Webtrends.multiTrack({
                        argsa: ['WT.si_n ', si_n, 'WT.si_s ', si_s, 'WT.si_x ', '99 ']
                    });
                } else if (data.status == -200 || data.status == -300) {

                    // content: "秒杀成功，奖品发放失败，失败原因＿data.desc,
                    // WT.si_x = -99 办理失败
                    Webtrends.multiTrack({
                        argsa: ['WT.si_n ', si_n, 'WT.si_s ', si_s, 'WT.si_x ', ' - 99 ', 'WT.errCode ', data.desc]
                    });
                } else {
                    $('.rule-item-content').append((new Date().getMinutes()) + ":" + (new Date().getSeconds()) + ":  seckillstatus:" + data.status + " desc:" + data.desc + type + "<br>");
                    // WT.si_x = -99 办理失败
                    Webtrends.multiTrack({
                        argsa: ['WT.si_n ', si_n, 'WT.si_s ', si_s, 'WT.si_x ', ' - 99 ', 'WT.errCode ', data.desc]
                    });
